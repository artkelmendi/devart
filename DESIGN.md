...
- **Entrance and reveal system:** The page opens with a short boot shutter, a restrained binary status signal, and a vertical scan. As it clears, the wordmark assembles through a masked 3D resolve, the binary field comes into focus, the role tracking tightens, and supporting content is uncovered in sequence. Scroll sections use distinct but related choreography: traced rules, directional masks, diagonal image wipes, controlled blur, and typographic construction. The sequence uses GSAP timelines and one-time ScrollTriggers so motion communicates structure without replaying on every scroll.
- **Motion controls:** The hero’s pause button freezes the binary field and exposes its state with `aria-pressed`. Its label alternates between “Motion on” and “Motion paused.” This control applies to the continuously moving background field; entrance and scroll reveals remain finite. The scroll cue loops over `2.7s`.
- **Reduced motion:** A live system-preference listener renders a static binary frame, resets pointer and scroll offsets, and prevents animation scheduling. The motion button becomes disabled and reads “Reduced motion.” CSS hides the entry shutter, removes animations and transitions, uses immediate anchor scrolling, and removes hover scale/lift effects. GSAP enhancement is skipped, leaving content in its normal visible state. Decorative canvas content is hidden from assistive technology and never captures pointer events.
