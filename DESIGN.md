...
The circle unfolds only into **</>** and returns to its moving orbit. Both forms use the same 800 binary particles in a 27-second cycle. The code fills the available sculpture area and smoothly changes between 80% and 100% scale, taking five seconds in each direction. Pointer parallax, particle pressure, and click/tap pulses remain active while formed; hit testing follows its position. Below 1000px, the symbol fits between the hero actions and bottom controls. Shapes fade back to the ambient field when leaving the hero.
